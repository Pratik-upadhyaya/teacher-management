"use client";
// Shared wizard steps for the teacher application form -- used by both
// the first-time registration wizard (app/register/page.tsx) and the
// pending/rejected teacher's edit form
// (app/(portal)/pending-approval/page.tsx). AccountStep/OtpVerify are
// registration-only (email/phone verification + password setup) and
// aren't used by the edit flow, since email is locked post-registration
// and password changes go through the existing change-password flow.
...
// ── Shared helpers ────────────────────────────────────────────────
export function ic(errors: Record<string, string>, field: string) {
...
export function isNepaliOnly(value: string): boolean {
  return /^[\u0900-\u097F\s\-,\.]+$/.test(value.trim());
}

export function validateNepaliOnly(
  value: string,
...
// ── Ward No. validator ────────────────────────────────────────────
export function validateWardNo(
  value: string,
  errs: Record<string, string>,
  field: string
) {
...
export function computeAgeSixty(dobValue: string): string | null {
